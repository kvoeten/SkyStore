...
export default async function PublicItemPage({ params }: { params: Promise<{ itemId: string }> }) {
  ...
  const officialCustomer = overview?.official.filter((entry) => entry.side === "customer_pays" && familyIds.has(entry.itemId)).sort((left, right) =>
    (right.septims[1] / right.quantity[0]) - (left.septims[1] / left.quantity[0]))[0];
  const officialStorePays = overview?.official.filter((entry) => entry.side === "store_pays" && familyIds.has(entry.itemId)).sort((left, right) =>
    (right.septims[1] / right.quantity[0]) - (left.septims[1] / left.quantity[0]))[0];
  ...
  return <AppShell current="/guide" publicView><div className="page"><p className="eyebrow"><a href="/guide">PRICE GUIDE</a> / {item.category.toUpperCase()}</p>
    ...
    <div className="item-layout"><div className="stack"><section className="panel"><div className="panel-head"><div><p className="eyebrow">PUBLIC MARKET</p><h2>Prices</h2></div></div><div className="grid rates"><article className="rate"><p>Store selling price</p><b>{bundle(officialCustomer)}</b><small>What a customer pays</small></article><article className="rate"><p>Store buying price</p><b>{bundle(officialStorePays)}</b><small>What the store pays for stock</small></article><article className="rate"><p>Reported sale price</p><b>{marketPrice(estimateCustomer?.upperQuartile)}</b><small>Highest approved report</small></article>{itemRecipes[0] && <article className="rate"><p>Material cost</p><b>{itemRecipes[0].materialCost == null ? "Incomplete" : formatGold(itemRecipes[0].materialCost)}</b><small>Current component cost</small></article>}</div><p className="market-footnote">All published prices and approved reports are public.</p></section>
      <section className="panel"><div className="panel-head"><div><p className="eyebrow">PRICE TIMELINE</p><h2>Store selling trend</h2></div>{trend && <Status kind={trend.direction === "down" ? "warn" : trend.direction === "up" ? "good" : "pending"}>{trend.percent == null ? "New" : `${trend.percent > 0 ? "+" : ""}${trend.percent.toFixed(1)}%`}</Status>}</div>{customerLine ? <><svg className="public-trend-chart" viewBox="0 0 100 100" preserveAspectRatio="none" role="img" aria-label={`Store selling price history for ${item.displayName}`}><polyline className="customer-line" points={customerLine}/></svg><div className="chart-legend"><span className="customer">Store selling price</span></div></> : <div className="empty"><h2>First price recorded.</h2><p>A trend appears after two or more official price updates.</p></div>}</section>
