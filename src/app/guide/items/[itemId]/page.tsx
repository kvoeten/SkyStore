...
import { baseCostRules, catalogImages, catalogItems } from "@/db/schema";
...
export default async function PublicItemPage({ params }: { params: Promise<{ itemId: string }> }) {
  ...
  const now = new Date();
  const [item, image, overview, itemRecipes, usedIn, priceFamily, access, baseCost] = await Promise.all([
    ...
    db.select({ totalSeptims: baseCostRules.totalSeptims, quantity: baseCostRules.quantity, effectiveFrom: baseCostRules.effectiveFrom }).from(baseCostRules).where(and(
      eq(baseCostRules.itemId, itemId), lte(baseCostRules.effectiveFrom, now), or(isNull(baseCostRules.effectiveTo), gte(baseCostRules.effectiveTo, now))
    )).orderBy(desc(baseCostRules.effectiveFrom)).limit(1).then((rows) => rows[0])
  ]);
  if (!item) notFound();
  ...
  const materialBaseValue = baseCost ? baseCost.totalSeptims / baseCost.quantity : null;
  const materialBaseIsNewer = materialBaseValue != null && (!officialCustomer?.effectiveFrom || baseCost!.effectiveFrom >= new Date(officialCustomer.effectiveFrom));
  const value = reportedStreetValue ?? (materialBaseIsNewer ? materialBaseValue : marketReferenceValue({ officialCustomerPays: officialCustomerValue }) ?? materialBaseValue);
  ...
  return <AppShell current="/guide" publicView publicAccount={Boolean(access)} showAdmin={access?.globalRole === "platform_admin"}><div className="page"><p className="eyebrow"><a href="/guide">PRICE GUIDE</a> / {item.category.toUpperCase()}</p>
    ...
    </div><aside className="stack"><MerchantPriceHelper value={value}/><section className="card"><p className="eyebrow">CATALOG DETAILS</p><dl className="public-item-meta"><div><dt>Plugin</dt><dd>{item.plugin ?? "Manual"}</dd></div><div><dt>Form ID</dt><dd>{item.localFormId ?? "—"}</dd></div><div><dt>Editor ID</dt><dd>{item.editorId ?? "—"}</dd></div><div><dt>Material base cost</dt><dd>{baseCost ? `${formatGold(baseCost.totalSeptims / baseCost.quantity)} (${baseCost.totalSeptims}g / ${baseCost.quantity})` : "—"}</dd></div><div><dt>Game value</dt><dd>{item.value ?? "—"}</dd></div><div><dt>Weight</dt><dd>{item.weight ?? "—"}</dd></div></dl></section></aside></div>
  </div></AppShell>;
}
