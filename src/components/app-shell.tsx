...
export function AppShell({ children, current, identity, publicView = false, publicAccount = false, showAdmin = false, searchAction, searchStoreId, searchPublicView = false }: { children: ReactNode; current?: string; identity?: ShellIdentity; publicView?: boolean; publicAccount?: boolean; showAdmin?: boolean; searchAction?: string; searchStoreId?: string; searchPublicView?: boolean }) {
  ...
  return <div className="app-shell"><a className="skip-link" href="#content">Skip to content</a>
    <aside className="sidebar" aria-label="Primary navigation">
      ...
      <nav className="sidebar-navigation">
        ...
        <details className="sidebar-fold"><summary>Product categories</summary><div className="nav-list profession-nav">{categoryLinks.map(([href, text]) => <a key={href} href={contextualHref(href)} aria-current={current === href ? "page" : undefined}><span aria-hidden="true">◇</span>{text}</a>)}</div></details>
        <details className="sidebar-fold"><summary>Crafting professions</summary><div className="nav-list profession-nav">{professionLinks.map(([href, text]) => <a key={href} href={contextualHref(href)} aria-current={current === href ? "page" : undefined}><span aria-hidden="true">◇</span>{text}</a>)}</div></details>
        {(!publicView || showAdmin) && section("Admin", adminLinks)}
      </nav>
    ...
    <header className="mobile-bar"><Link href="/guide" className="brand"><img src="/brand/emblem.svg" alt="" width={30} height={30}/><span><strong>Sky</strong>Store</span></Link><button className="menu-button" type="button" aria-expanded={menuOpen} aria-controls="mobile-nav" onClick={() => setMenuOpen((open) => !open)}>Menu</button><nav id="mobile-nav" className={`mobile-nav ${menuOpen ? "open" : ""}`} aria-label="Mobile navigation">
      ...
      {(!publicView || showAdmin) && <><span className="mobile-nav-label">Admin</span>{adminLinks.map(([href,text,icon]) => <Link key={href} href={href} onClick={closeMenu}><span aria-hidden="true">{icon}</span>{text}</Link>)}{!publicView && <Link href="/guide?view=public" onClick={closeMenu}>Public price guide</Link>}{!publicView && <Link href="/logout" onClick={closeMenu}>Sign out</Link>}</>}
    ...
    <main id="content" className="content"><header className="topbar"><form className="global-search" role="search" action={searchAction ?? "/guide"} method="get">{effectiveSearchStoreId && !publicView && <input type="hidden" name="storeId" value={effectiveSearchStoreId}/>} {searchPublicView && <input type="hidden" name="view" value="public"/>}<span aria-hidden="true">⌕</span><input name="q" type="search" placeholder="Search catalog and prices" aria-label="Search catalog and prices"/><kbd>Enter</kbd></form><div className="top-actions">{publicView && <><Link className="outline quick-report" href="/guide/bulk-report">Bulk report</Link><Link className="outline quick-report" href="/guide/report-ledger">Report ledger</Link></>}{publicView ? publicAccount ? <><Link className="outline" href="/guide">Store view</Link><Link className="text-button" href="/logout">Sign out</Link></> : <Link className="outline" href={loginHref as Route}>Sign in</Link> : <><Link className="text-button" href="/guide?view=public">Public guide</Link><Link className="text-button" href="/logout">Sign out</Link></>}</div></header>{children}</main>
  </div>;
}
