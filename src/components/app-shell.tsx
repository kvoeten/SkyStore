...
export function AppShell({ children, current, identity, publicView = false, publicAccount = false, searchAction, searchStoreId, searchPublicView = false }: { children: ReactNode; current?: string; identity?: ShellIdentity; publicView?: boolean; publicAccount?: boolean; searchAction?: string; searchStoreId?: string; searchPublicView?: boolean }) {
  ...
  const professionHref = (href: string) => !publicView && effectiveSearchStoreId ? `${href}?storeId=${encodeURIComponent(effectiveSearchStoreId)}` : searchPublicView ? `${href}?view=public` : href;
  return <div className="app-shell"><a className="skip-link" href="#content">Skip to content</a>
    <aside className="sidebar" aria-label="Primary navigation">
      ...
      <nav className="sidebar-navigation"><div className="nav-list">{visibleLinks.map(([href, label, icon]) => <Link key={href} href={href} aria-current={current === href ? "page" : undefined}><span aria-hidden="true">{icon}</span>{label}</Link>)}</div><p className="nav-section-label">Professions</p><div className="nav-list profession-nav">{professionLinks.map(([href, label]) => <a key={href} href={professionHref(href)} aria-current={current === href ? "page" : undefined}><span aria-hidden="true">◇</span>{label}</a>)}</div></nav>
