...
export function BulkPriceReport() {
  ...
  async function submit() {
    ...
    const results = await Promise.all(ready.map(({ row, price }) => fetch("/api/v1/market/reports", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ itemId: row.item!.id, quantity: 1, totalSeptims: price, priceType: row.priceType, sourceLocation: row.location, occurrenceAt: row.occurredAt }) })));
