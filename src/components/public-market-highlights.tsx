...
function highestPublicPrice(state: Highlights, entry: HighlightLink): number | null {
  ...
  const estimates = state.estimates.filter((estimate) => estimate.side === "customer_pays" && matches(estimate)).map((estimate) => Number(estimate.upperQuartile ?? estimate.median)).filter(Number.isFinite);
