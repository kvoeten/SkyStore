...
export function PublicMarketReportReview() {
  ...
  return <section className="panel" style={{ marginTop: 20 }}>
    ...
    {reports?.length ? <ul className="list">{reports.map((report) => <li key={report.id}>
      <span><b>{report.itemName}</b><br/><small>{formatGold(report.totalSeptims / report.quantity)} · {report.quantity} units · {formatGold(report.totalSeptims)} total · {report.locationType === "store_sale" ? "Store sale" : "Street sale"}</small><br/><small>{report.contributorDisplayName} · Discord: {report.contributorDiscordName ?? "Unknown"}{report.contributorDiscordId ? ` (${report.contributorDiscordId})` : ""} · {new Date(report.createdAt).toLocaleString()}</small>{report.note ? <><br/><small>{report.note}</small></> : null}</span>
    ...
    </li>)}</ul> : reports ? <div className="empty"><h3>No public reports pending.</h3><p>New Discord-authenticated reports will appear here for platform review.</p></div> : <p>Loading reports…</p>}
